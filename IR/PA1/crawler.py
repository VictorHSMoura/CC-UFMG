...
if __name__ == "__main__":
    ...
    for f, size in enumerate([1, 2, 5, 10, 12, 25, 50, 100]):
        ...
        for i in range(5):
            ...
            # print(f"File 0")
            outFile = open(f'corpus/crawl{(5*f)+i}.warc.gz', 'wb')
            writer = WARCWriter(outFile, gzip=True)

            start = time()
            threads = [Crawler(i, limit, debug) for i in range(size)]
